...
if df_train_val_national.empty:
    print("No training/validation data for national study. Cannot proceed.")
else:
    splits = get_splits(df_train_val_national, validation_strategy)

    study_name = f"lstm-national-regressor-{validation_strategy}-{landuse_suffix}-{VERSION_STAMP}"
    ...
    study.optimize(
        lambda trial: objective(
            trial,
            X=X_train_val_national,
            y=y_train_val_national,
            splits=splits,
